...
public class Arkanoid {
    ...
    public Arkanoid(final int width, final int height) {
        this.width = width;
        this.height = height;
    }
    ...
    void run() throws Exception {
        ...
        final KeyboardObserver keyboardObserver = new KeyboardObserver();
    ...
    /**
     * Проверяем столкновение с кирпичами. Если столкновение было - шарик отлетает в
     * случайном направлении 0..360 градусов
     */
    void checkBricksBump() {
        for (final Brick brick : new ArrayList<Brick>(bricks)) {
    ...
    /**
     * Проверяем столкновение с подставкой. Если столкновение было - шарик отлетает
     * в случайном направлении вверх 80..100 градусов.
     */
    void checkStandBump() {
        if (ball.isIntersec(stand)) {
            final double angle = 90 + 20 * (Math.random() - 0.5);
            ball.setDirection(angle);
        }
    }

    /**
     * Проверяем - не улетел ли шарик через дно. Если да - игра окончена (isGameOver
     * = true)
     */
    void checkEndGame() {
    ...
    public static void main(final String[] args) throws Exception {
        ...
        final Ball ball = new Ball(10, 29, 2, 95);
        game.setBall(ball);

        final Stand stand = new Stand(10, 30);
        game.setStand(stand);
